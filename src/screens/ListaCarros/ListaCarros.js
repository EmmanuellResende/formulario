...
export default function ListaCarros() {
    ...
    const [carros, setCarros] = useState(["Porsche", "Fusca"])
    const [inputValue, setInputValue] = useState('')
    const [editando, setEditando] = useState(false)
    const [carroSendoEditado, setCarroSendoEditado] = useState(null)

    function adicionarCarro() {
        let novaListaCarros = carros.slice() // Use slice para criar uma nova cópia do array
                ...
                renderItem={({ item }) => (
                    ...
                    >
                        <Card.Content style={styles.cardContent}>
                            ...
                            <IconButton icon='pencil' onPress={() => handleEditarCarro(item)} />
                            <IconButton icon='delete' onPress={() => excluirCarro(item)} />
                        </Card.Content>
                    </Card>
                )}
            />
